...
df = pd.read_csv('../../datasets/mendeley_enfermedades/silver/01_pca_40comp.csv')
...
ax1.set_title('MENDELEY - Top 20 Componentes por Score MI', fontweight='bold', fontsize=12)
ax1.invert_yaxis()
...
ax2.plot(range(1, 41), mutual_information['Score'].values, linewidth=2, color='steelblue', marker='o', markersize=3)
...
ax2.set_title('Distribución de Scores (40 componentes)', fontweight='bold', fontsize=12)
ax2.legend()
ax2.grid(True, alpha=0.3)
ax2.set_xlim(1, 40)

plt.suptitle('MENDELEY - Análisis de Mutual Information: Selección de Componentes', fontsize=14, fontweight='bold')
plt.tight_layout()
plt.savefig('../../resultados/mendeley_enfermedades/graficos/02_mutual_information_analisis.png', dpi=200, bbox_inches='tight')
...
Reducción:              40 → 10 componentes (75% menos)
...
- Esta selección reduce la dimensionalidad de 40 a 10 componentes,
  manteniendo las más relevantes para la clasificación.
"""

with open('../../resultados/mendeley_enfermedades/informes/03_informe_mutual_information.txt', 'w') as f:
    f.write(informe)